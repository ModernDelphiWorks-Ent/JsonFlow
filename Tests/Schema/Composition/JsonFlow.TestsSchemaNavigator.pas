...
begin
  ...
  Assert.IsNotNull(LElement, 'Deveria encontrar o elemento no �ndice do array');
  Assert.IsTrue(Supports(LElement, IJSONValue, LValue), 'Deveria ser um valor');
  Assert.AreEqual('integer', LValue.AsString, 'Valor esperado no �ndice 1');
end;
...
begin
  ...
  Assert.IsNotNull(LObj, 'Deveria retornar um objeto v�lido');
...
begin
  ...
  Assert.IsNull(LObj, 'Deveria retornar nil para um caminho inv�lido');
end;
...
begin
  ...
  Assert.IsNotNull(LArr, 'Deveria retornar um array v�lido');
...
begin
  ...
  Assert.IsNull(LArr, 'Deveria retornar nil para um caminho que n�o � array');
end;
...
begin
  ...
  Assert.IsNotNull(LElement, 'Deveria resolver a refer�ncia local em $defs');
  Assert.IsTrue(Supports(LElement, IJSONObject, LObj), 'Deveria ser um objeto');
  Assert.AreEqual('string', (LObj.GetValue('type') as IJSONValue).AsString, 'Valor esperado na refer�ncia');
end;
...
begin
  ...
  Assert.IsNotNull(LElement, 'Deveria resolver a refer�ncia local em $anchor');
  Assert.IsTrue(Supports(LElement, IJSONObject, LObj), 'Deveria ser um objeto');
  Assert.AreEqual('string', (LObj.GetValue('type') as IJSONValue).AsString, 'Valor esperado na refer�ncia');
end;
...
begin
  ...
  try
    ...
    Assert.IsNotNull(LElement, 'Deveria resolver a refer�ncia externa');
    Assert.IsTrue(Supports(LElement, IJSONObject, LObj), 'Deveria ser um objeto');
    Assert.AreEqual('string', (LObj.GetValue('type') as IJSONValue).AsString, 'Valor esperado na refer�ncia externa');
  finally
...
begin
  ...
  Assert.IsNull(LElement, 'Deveria retornar nil para um caminho inv�lido');
end;
