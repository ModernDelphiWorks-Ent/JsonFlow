unit JsonFlow4D.TestsSchemaComposerDuplicate;
...
begin
  ...
  LValidJSON := '{"homeAddress": {"street": "Main St", "city": "New York"}, "type": "person", "name": "Jo�o", "status": "active", "score": 50, "tags": ["abc", "def"]}';
...
begin
  ...
  LInvalidJSON := '{"mainContact": {}}'; // JSON inv�lido pra testar o required
  Writeln('Invalid JSON: ' + LInvalidJSON);
  LValidator := TJSONSchemaValidator.Create(jsvDraft7); // Deixa a lista padr�o de traits como est�
  try
    LResult := LValidator.Validate(LInvalidJSON, LJSONSchema);
    if not LResult then
      Writeln('Validation errors (invalid): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
...
begin
  ...
  LValidator := TJSONSchemaValidator.Create(jsvDraft7);
  try
    // Voc� habilita/desabilita os traits no _InitializeTraits ou aqui
...
begin
  ...
  LValidator := TJSONSchemaValidator.Create(jsvDraft7);
  try
    // Traits j� est�o controlados no _InitializeTraits
    LResult := LValidator.Validate(LJSON, LJSONSchema);
    if not LResult then
      Writeln('Validation errors: ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
    Writeln('Result: ' + BoolToStr(LResult, True));
...
begin
  ...
  // Constru��o gradual do meta-schema: s� type
  ...
  try
    ...
    if not LResult then
      Writeln('Validation errors: ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
    Assert.IsTrue(LResult, 'Caso v�lido 1 deveria passar');
    Writeln('Result: ' + BoolToStr(LResult, True));
...
begin
  ...
  // Constru��o gradual do meta-schema: type + $defs + $ref b�sico
  FComposer.Clear; // Limpa estado anterior
  FComposer.Obj
    .Typ('object')
    .Def('contact', procedure(D: TJSONSchemaComposer) begin D.Typ('object'); end)
...
begin
  ...
  // Constru��o gradual do meta-schema: type + $defs + $ref + required
  FComposer.Clear; // Limpa estado anterior
  FComposer.Obj
    .Typ('object')
    .Def('contact', procedure(D: TJSONSchemaComposer) begin D.Typ('object'); end)
    .Prop('mainContact', procedure(P: TJSONSchemaComposer) begin P.Add('$ref', '#/$defs/contact'); end)
    .RequiredFields(['mainContact']) // Adiciona a propriedade como obrigat�ria
    .EndObj;

  // JSON v�lido (com mainContact)
  LJSONValid := '{"mainContact": {}}';
  Writeln('JSON v�lido: ' + LJSONValid);

  // JSON inv�lido (sem mainContact)
  LJSONInvalid := '{}';
  Writeln('JSON inv�lido: ' + LJSONInvalid);

  // Parse dos JSONs para IJSONElement
  LReader := TJsonReader.Create;
  try
    LJSONValidElement := LReader.Read(LJSONValid);
    Assert.IsNotNull(LJSONValidElement, 'Falha ao parsear o JSON v�lido');
    Writeln('Parsed JSON v�lido: ' + LJSONValidElement.AsJSON);

    LJSONInvalidElement := LReader.Read(LJSONInvalid);
    Assert.IsNotNull(LJSONInvalidElement, 'Falha ao parsear o JSON inv�lido');
    Writeln('Parsed JSON inv�lido: ' + LJSONInvalidElement.AsJSON);
  finally
    LReader.Free;
  end;
  ...
  try
    ...
    Writeln('Validando JSON v�lido...');
    LResult := LValidator.Validate(LJSONValidElement, '');
    if not LResult then
      Writeln('Validation errors (v�lido): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
    Assert.IsTrue(LResult, 'Caso v�lido 1 deveria passar');
    Writeln('Result (v�lido): ' + BoolToStr(LResult, True));

    // Teste com JSON inv�lido
    Writeln('Validando JSON inv�lido...');
    LResult := LValidator.Validate(LJSONInvalidElement, '');
    if not LResult then
      Writeln('Validation errors (inv�lido): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
    Assert.IsFalse(LResult, 'Caso inv�lido 1 deveria falhar');
    Writeln('Result (inv�lido): ' + BoolToStr(LResult, True));
...
begin
  ...
  try
    ...
    Assert.IsNotNull(LJSONValid, 'Falha ao parsear JSON v�lido');
    Assert.IsNotNull(LJSONInvalid1, 'Falha ao parsear JSON inv�lido 1');
    Assert.IsNotNull(LJSONInvalid2, 'Falha ao parsear JSON inv�lido 2');
    Writeln('JSON v�lido: ' + LJSONValid.AsJSON);
    Writeln('JSON inv�lido 1 (sem age): ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2 (age negativo): ' + LJSONInvalid2.AsJSON);
    ...
    try
      ...
      Writeln('Validando JSON inv�lido 1...');
      LResult := LValidator.Validate(LJSONInvalid1, ''); // Reutiliza o schema
      if not LResult then
        Writeln('Validation errors (inv�lido 1): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
      Assert.IsFalse(LResult, 'Caso inv�lido 1 deveria falhar');
      Writeln('Result (inv�lido 1): ' + BoolToStr(LResult, True));

      Writeln('Validando JSON inv�lido 2...');
      LResult := LValidator.Validate(LJSONInvalid2, ''); // Reutiliza o schema
      if not LResult then
        Writeln('Validation errors (inv�lido 2): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
      Assert.IsFalse(LResult, 'Caso inv�lido 2 deveria falhar');
      Writeln('Result (inv�lido 2): ' + BoolToStr(LResult, True));
...
//    Assert.IsNotNull(LJSONInvalid2, 'Falha ao parsear JSON inv�lido 2');
//    Writeln('JSON v�lido: ' + LJSONValid.AsJSON);
//    Writeln('JSON inv�lido 1 (age negativo no array): ' + LJSONInvalid1.AsJSON);
//    Writeln('JSON inv�lido 2 (sem age no array): ' + LJSONInvalid2.AsJSON);
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido: ' + LJSONValid.AsJSON);
    Writeln('JSON inv�lido 1 (array vazio): ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2 (role fora do enum): ' + LJSONInvalid2.AsJSON);
    Writeln('JSON inv�lido 3 (array longo): ' + LJSONInvalid3.AsJSON);
    ...
    try
      ...
      Writeln('Validando JSON inv�lido 3...');
      LResult := LValidator.Validate(LJSONInvalid3, '');
      if not LResult then
        Writeln('Validation errors (inv�lido 3): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
      Assert.IsFalse(LResult, 'Caso inv�lido 3 deveria falhar');
      Writeln('Result (inv�lido 3): ' + BoolToStr(LResult, True));
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido: ' + LJSONValid.AsJSON);
    Writeln('JSON inv�lido 1 (email inv�lido): ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2 (array vazio): ' + LJSONInvalid2.AsJSON);
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido: ' + LJSONValid.AsJSON);
    Writeln('JSON inv�lido 1 (sem phone): ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2 (phone inv�lido): ' + LJSONInvalid2.AsJSON);
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido 1: ' + LJSONValid1.AsJSON);
    Writeln('JSON v�lido 2: ' + LJSONValid2.AsJSON);
    Writeln('JSON inv�lido (nenhum anyOf v�lido): ' + LJSONInvalid.AsJSON);
    ...
    try
      ...
      Writeln('Validando JSON v�lido 1...');
      LResult := LValidator.Validate(LJSONValid1, '');
      if not LResult then
        Writeln('Validation errors (v�lido 1): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
      Assert.IsTrue(LResult, 'Caso v�lido 1 deveria passar');
      Writeln('Result (v�lido 1): ' + BoolToStr(LResult, True));

      Writeln('Validando JSON v�lido 2...');
      LResult := LValidator.Validate(LJSONValid2, '');
      if not LResult then
        Writeln('Validation errors (v�lido 2): ' + TArray.ToString<TValidationError>(LValidator.GetErrors));
      Assert.IsTrue(LResult, 'Caso v�lido 2 deveria passar');
      Writeln('Result (v�lido 2): ' + BoolToStr(LResult, True));
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido 1: ' + LJSONValid1.AsJSON);
    Writeln('JSON v�lido 2: ' + LJSONValid2.AsJSON);
    Writeln('JSON inv�lido 1 (nenhum valida): ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2 (ambos validam): ' + LJSONInvalid2.AsJSON);
...
begin
  ...
  try
    ...
    Writeln('JSON v�lido 1: ' + LJSONValid1.AsJSON);
    Writeln('JSON v�lido 2: ' + LJSONValid2.AsJSON);
    Writeln('JSON v�lido 3: ' + LJSONValid3.AsJSON);
    Writeln('JSON inv�lido 1: ' + LJSONInvalid1.AsJSON);
    Writeln('JSON inv�lido 2: ' + LJSONInvalid2.AsJSON);
    ...
    try
      ...
      Writeln('Validando JSON v�lido 3...');
      LResult := LValidator.Validate(LJSONValid3, '');
      Writeln('Result (v�lido 3): ' + BoolToStr(LResult, True));
      if not LResult then
        for I := 0 to Length(LValidator.GetErrors) - 1 do
          Writeln('  Erro ' + IntToStr(I + 1) + ': ' + LValidator.GetErrors[I].Message);
...
begin
  ...
  Writeln('Schema constru�do, gerando JSON');
  LGeneratedSchema := FComposer.ToElement;
  Writeln('Schema: ' + LGeneratedSchema.AsJSON);
...
//    Assert.IsTrue(Schema.Validate(JSONString, Errors), 'JSON v�lido deve passar');
...
//    // Adicionar verifica��o mais robusta
...
//        raise EAssertionFailed.Create('FMetaSchema n�o est� atribu�do');
...
//      Assert.IsTrue(LResult, 'O schema gerado deve ser v�lido contra o meta-schema');
//      Assert.AreEqual(0, Length(LSchemaValidator.GetErrors), 'Nenhum erro de valida��o esperado');
//    finally
...
//          '"type": "person", "name": "Jo�o", "status": "active", ' +
...
//          'JSON v�lido deve passar na valida��o com schema com coment�rio');
//      finally
//        LReader.Free;
//      end;
//    finally
