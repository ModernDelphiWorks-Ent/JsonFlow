unit JsonFlow4D.TestsSchemaReader;
...
begin
  ...
  Assert.IsTrue(LResult, 'LoadFromString deveria retornar True para um schema v�lido');
  Assert.IsNotNull(FReader.GetVersion, 'Vers�o deveria ser detectada');
end;
...
var
  ...
  LResult: Boolean;
begin
  LSchema := '{type: "object"'; // JSON inv�lido (falta fechar chaves)
  LResult := FReader.LoadFromString(LSchema);
  Assert.IsFalse(LResult, 'LoadFromString deveria retornar False para JSON inv�lido');
...
begin
  ...
  try
    ...
    Assert.IsTrue(LResult, 'LoadFromFile deveria retornar True para um arquivo v�lido');
  finally
...
begin
  ...
  Assert.IsTrue(LResult, 'Validate deveria retornar True para uma inst�ncia v�lida');
  Assert.AreEqual(0, Length(FReader.GetErrors), 'N�o deveria haver erros');
end;
...
begin
  ...
  FReader.LoadFromString(LSchema);
  LResult := FReader.Validate(LJson);
  Assert.IsFalse(LResult, 'Validate deveria retornar False para uma inst�ncia inv�lida');
...
begin
  ...
  FReader.LoadFromString(LSchema);
  Assert.AreEqual(jsvDraft7, FReader.GetVersion, 'Deveria defaultar pra Draft 7 pra vers�o desconhecida');
  Assert.AreEqual(1, Length(FReader.GetErrors), 'Deveria registrar um erro pra vers�o desconhecida');
end;
...
begin
  ...
  Assert.IsTrue(LResult, 'Validate com schema string deveria retornar True para inst�ncia v�lida');
  Assert.AreEqual(0, Length(FReader.GetErrors), 'N�o deveria haver erros');
end;
